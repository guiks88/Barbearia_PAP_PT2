...
function getAvailableStock(product) {
  const total = getSafeStock(product)
...
function renderProductCards(container, products) {
  ...
  container.innerHTML = products
    .map((product) => {
      ...
      const stockLabel = outOfStock ? 'Esgotado' : `Stock disponível: ${stock}`
        ...
        <div class="product-card ${outOfStock ? 'is-out-of-stock' : ''}">
          ...
          <div class="product-info">
            ...
            <div class="product-actions">
              <input type="number" class="product-qty-input" data-product-id="${product.id}" min="1" max="${Math.max(1, stock)}" value="1" ${outOfStock ? 'disabled' : ''}>
  ...
  container.querySelectorAll('.product-add').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (!productId) return
      const qtyInput = container.querySelector(`.product-qty-input[data-product-id="${productId}"]`)
      const requestedQty = Math.max(1, parseInt(qtyInput?.value || '1', 10) || 1)
      if (!isClientLoggedInForCart()) {
        showError('Não é possível adicionar produto. Inicie sessão para usar o carrinho.')
        return
      }
      sessionStorage.setItem('pendingShopProductId', productId)
      sessionStorage.setItem('pendingShopProductQty', String(requestedQty))
...
function initShopCart() {
  ...
  const featuredContainerInitial = document.getElementById('featuredProductsGrid')
  if (featuredContainerInitial) {
    const featured = [...shopProductsCache].sort((a, b) => (b.salesCount || 0) - (a.salesCount || 0)).slice(0, 4)
    renderProductCards(featuredContainerInitial, featured)
  }
  ...
  if (shopLogoutBtn && shopLogoutBtn.dataset.bound !== 'true') {
    ...
    shopLogoutBtn.addEventListener('click', async () => {
      try {
        ...
        console.error('Erro ao terminar sessÃ£o na loja:', error)
      }
      clearClientSession()
      window.location.href = 'index.html'
    })
...
function addToCart(product, requestedQty = 1) {
  if (!isClientLoggedInForCart()) {
    showError('NÃ£o Ã© possÃ­vel adicionar produto sem sessÃ£o iniciada. Inicie sessÃ£o para continuar.')
    return { added: 0, requested: requestedQty, accepted: 0 }
  }
  if (!product?.id) return { added: 0, requested: requestedQty, accepted: 0 }
  const available = getAvailableStock(product)
  if (available <= 0) return { added: 0, requested: requestedQty, accepted: 0 }

  const desiredQty = Math.max(1, Number.parseInt(requestedQty, 10) || 1)
  const acceptedQty = Math.min(desiredQty, available)
  ...
  return { added: acceptedQty, requested: desiredQty, accepted: acceptedQty }
}
...
function renderShopProducts() {
  ...
  container.querySelectorAll('.product-add').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (!product) return
      const qtyInput = container.querySelector(`.product-qty-input[data-product-id="${id}"]`)
      const requestedQty = Math.max(1, parseInt(qtyInput?.value || '1', 10) || 1)
      ...
      if (result.added < result.requested) {
        showSuccess(`Foram adicionados ${result.added} produtos (limite de stock disponível).`)
      } else {
        showSuccess(`Foram adicionados ${result.added} produtos ao carrinho.`)
      }
    })
  })
}
...
function loadProducts() {
  ...
  onValue(ref(database, 'products'), (snapshot) => {
    ...
    if (shopContainer && pendingProductId) {
      ...
      const pendingQtyRaw = sessionStorage.getItem('pendingShopProductQty')
      const pendingQty = Math.max(1, parseInt(pendingQtyRaw || '1', 10) || 1)
      if (!isClientLoggedInForCart()) {
        showError('Não é possível adicionar produto sem login. Inicie sessão e tente novamente.')
      } else if (pendingProduct && getSafeStock(pendingProduct) > 0) {
        const result = addToCart(pendingProduct, pendingQty)
        if (result.added > 0) {
          if (result.added < result.requested) {
            showSuccess(`Produto pré-selecionado: adicionados ${result.added} (limite de stock disponível).`)
          } else {
            showSuccess(`Produto pré-selecionado: adicionados ${result.added} ao carrinho.`)
          }
        }
      }
      sessionStorage.removeItem('pendingShopProductId')
      sessionStorage.removeItem('pendingShopProductQty')
