...
function hasBookingStartTimePassed(dateStr, timeStr) {
...
function getMinutesUntilBookingStart(booking) {
  ...
  return Math.floor((bookingDate.getTime() - Date.now()) / 60000)
}
...
function isBookingLockedForClientChanges(booking) {
  ...
  if (booking?.executionStatus === 'completed') return true

  const minutesLeft = getMinutesUntilBookingStart(booking)
  if (minutesLeft === null) return false

  // One hour or less before the appointment, client actions are blocked.
...
async function refreshRescheduleTimeOptions(bookingId) {
  ...
  if (isBookingLockedForClientChanges(booking)) {
    showError('Falta 1 hora ou menos para a marcacao. Ja nao pode adiar.')
    return
  }
  ...
  for (const slot of defaultWorkingHours) {
    ...
    const ownConflict = await hasClientBookingConflict(booking.clientUid, selectedDate, slot, bookingId, durationMinutes)
...
function isBookingExpiredForClient(booking) {
  ...
  if (booking.date === todayStr && hasBookingStartTimePassed(booking.date, booking.time)) {
    return true
  }
...
async function hasClientBookingConflict(clientUid, date, time, bookingIdToIgnore, durationMinutes = 30) {
  ...
  return Object.entries(allBookings).some(([id, booking]) => {
    if (!booking || id === bookingIdToIgnore) return false
    if (booking.clientUid !== clientUid) return false
    if (booking.status === 'cancelled' || booking.status === 'expired') return false
...
function renderClientBookings() {
  ...
  listEl.innerHTML = filteredBookings.map((booking) => {
    ...
    const isLockedByTime = !isCancelled && !isCompleted && isBookingLockedForClientChanges(booking)
    const isLocked = isCancelled || isCompleted || isLockedByTime
    ...
    return `
      <div class="client-booking-card ${isCancelled ? 'is-cancelled' : ''}" data-booking-id="${booking.id}">
        <div class="client-booking-main">
          ...
          ${isLockedByTime ? '<p style="color: var(--color-error);"><strong>Aviso:</strong> Falta 1 hora ou menos para este horario. Ja nao pode adiar nem anular.</p>' : ''}
        </div>
        ${isLocked ? '' : `
...
async function rescheduleBooking(bookingId) {
  ...
  if (isBookingLockedForClientChanges(booking)) {
    showError('Falta 1 hora ou menos para a marcação. Já não pode adiar.')
    return
  }
  ...
  try {
    ...
    const ownConflict = await hasClientBookingConflict(booking.clientUid, newDate, newTime, bookingId, currentDuration)
    if (ownConflict) {
      showError('Já tem outra marcação nesse horário com outro barbeiro.')
      return
    }
...
async function cancelBookingByClient(bookingId) {
  ...
  if (isBookingLockedForClientChanges(booking)) {
    showError('Falta 1 hora ou menos para a marcação. Já não pode anular.')
    return
  }
...
async function confirmBooking() {
  ...
  try {
    ...
    const ownConflict = await hasClientBookingConflict(currentUser.uid, bookingState.date, bookingState.time, null, selectedDuration)
    if (ownConflict) {
      showError('Já tem outra marcação nesse horário. Escolha outro horário.')
      document.getElementById('step-client-data').classList.add('hidden')
      document.getElementById('step-datetime').classList.remove('hidden')
      renderCalendar()
      renderTimeSlots(bookingState.date)
